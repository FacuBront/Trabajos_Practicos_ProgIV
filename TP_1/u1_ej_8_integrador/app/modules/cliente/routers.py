...
def baja_logica_cliente(id: int = Path(..., gt=0)):
    try:
...
def pagar_saldo(id: int = Path(..., gt=0), monto: float = Query(..., gt=0)):
    try:
        cliente_actualizado = services.registrar_pago(id=id, monto=monto)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))
...
def estado_credito(id: int = Path(..., gt=0)):
    estado = services.obtener_estado_credito(id)
    if not estado:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Cliente no encontrado"
        )
